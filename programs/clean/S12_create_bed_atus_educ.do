...
preserve
	collapse (sum) duration, by(cpsidp qtr industry)

	keep cpsidp qtr industry
	gen education = 1
...
merge m:1 cpsidp qtr industry using `education'
drop _merge 

*1.12: take the sum of time spent working for each individual
collapse (sum) duration (first) year education wt06 wt20, by(cpsidp industry qtr where)

*1.13: define online learning to be at home or someone else's home
...
preserve 
	keep if year == 2020 
	collapse (sum) education online_educ_atus [fw=wt20], by(qtr industry)
	tempfile 2020
	save `2020', replace
restore

*1.14: drop duration less than 1 hour
drop if duration < 60

*1.15: calculate minutes spent learning and online learning by industry year
collapse (sum) education online_educ_atus [fw=wt06], by(qtr industry)
append using `2020'

*1.16: calculate the online learning rate, defined to be: (online learning time/total time spent learning) by super-sector quarter. 
gen online_educ_rate = online_educ_atus/ education
sort qtr industry
keep qtr industry online_educ_rate

*1.17: prepare data to merge with BED. 
rename qtr t 
tostring t, gen(time_temp)
destring time_temp, replace
destring industry, gen(panel_variable)

*1.18: merge in BED super sector data: 
merge 1:1 t industry using "$bed/bed_industry.dta", keep(3) nogen
/* not matched from using are the following:
pre 2003q1
post 2022q4
*/

save "$bed/bed_atus_educ.dta", replace
