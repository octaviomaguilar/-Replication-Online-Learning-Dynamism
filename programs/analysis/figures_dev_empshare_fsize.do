...
keep t time statefips naics4 firmsize emp frm* hir* sep* bartik
reshape wide emp frm* hir* sep*, i(t naics4 bartik statefips) j(firmsize)
...
**(2)**
*******
*Block 2 produces figure 4 panel A
*Block 3.1-3.3 produces figure 5 Panels A,B, and E. 

use "$data/bartik/bartik_state_naics4_fsize_trimmed.dta", clear

*Figure 4 Panel A: Deviation in employment share for firms with 500+ employees:
qui reghdfe dev_emp_share5 ib239.t##c.bartik, absorb(i.Istate i.Inaics4 i.Inaics2_time) vce(cluster Inaics4 time)
	...
	twoway (rarea min95 max95 time, fcolor(Blueish-gray8*0.15) lcolor(purple*0.15) lw(none) lpattern(solid)) ///
	       ...
	       ylabel(, format(%9.2f))
		restore
		graph export "$figures/figure4_panelA.eps", replace

*********
**(3.1)**
*********
*Figure 5 Panel A: Deviation in employment share for firms with 0-19 employees:
qui reghdfe dev_emp_share1 ib239.t##c.bartik, absorb(i.Istate i.Inaics4 i.Inaics2_time) vce(cluster Inaics4 time)
	...
	twoway (rarea min95 max95 time, fcolor(Blueish-gray8*0.15) lcolor(purple*0.15) lw(none) lpattern(solid)) ///
	       ...
	       ylabel(, format(%9.2f))
		restore
		graph export "$figures/figure5_panelA.eps", replace
	
*********
**(3.2)**
*********
*Figure 5 Panel B: Deviation in employment share for firms with 20-49+ employees:
qui reghdfe dev_emp_share2 ib239.t##c.bartik, absorb(i.Istate i.Inaics4 i.Inaics2_time) vce(cluster Inaics4 time)
	...
	twoway (rarea min95 max95 time, fcolor(Blueish-gray8*0.15) lcolor(purple*0.15) lw(none) lpattern(solid)) ///
	       ...
	       ylabel(, format(%9.2f))
		restore
		graph export "$figures/figure5_panelB.eps", replace

*********
**(3.3)**
*********
*Figure 5 Panel E: Deviation in employment share for firms with 250-499+ employees:
qui reghdfe dev_emp_share4 ib239.t##c.bartik, absorb(i.Istate i.Inaics4 i.Inaics2_time) vce(cluster Inaics4 time)
	...
	twoway (rarea min95 max95 time, fcolor(Blueish-gray8*0.15) lcolor(purple*0.15) lw(none) lpattern(solid)) ///
	       ...
	       ylabel(, format(%9.2f))
		restore
		graph export "$figures/figure5_panelE.eps", replace
