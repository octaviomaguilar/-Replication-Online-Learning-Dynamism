...
*******
*load QWI data
use "$data/bartik/bartik_qwi_earn_fsize.dta", clear
drop earnseps
gen t = time 
format t %tq
keep t time statefips naics4 firmsize earn* bartik

*1.1: reshape the data
reshape wide earn*, i(t naics4 bartik statefips) j(firmsize)
...
save "$data/bartik/bartik_qwi_earn_fsize_trimmed.dta", replace

*******
**(2)**
*******
use "$data/bartik/bartik_qwi_earn_fsize_trimmed.dta", clear

*Figure 6 panel A: deviation in average monthly earnings for firm with 0-19 employees:
qui reghdfe dev_earnbeg1 ib239.t##c.bartik, absorb(i.Istate i.Inaics4 i.Inaics2_time) vce(cluster Inaics4 time)
	...
	twoway (rarea min95 max95 time, fcolor(Blueish-gray8*0.15) lcolor(purple*0.15) lw(none) lpattern(solid)) ///
	       ...
	       ylabel(, format(%9.2f))
		restore
		graph export "$figures/figure6_panelA.eps", replace

*********
**(2.1)**
*********
*Figure 6 panel B: deviation in average monthly earnings for firm with 20-49 employees:
qui reghdfe dev_earnbeg2 ib239.t##c.bartik, absorb(i.Istate i.Inaics4 i.Inaics2_time) vce(cluster Inaics4 time)
	...
	twoway (rarea min95 max95 time, fcolor(Blueish-gray8*0.15) lcolor(purple*0.15) lw(none) lpattern(solid)) ///
	       ...
	       ylabel(, format(%9.2f))
		restore
		graph export "$figures/figure6_panelB.eps", replace

*********
**(2.2)**
*********
*Figure 6 panel C: deviation in average monthly earnings for firm with 50-249 employees:
qui reghdfe dev_earnbeg3 ib239.t##c.bartik, absorb(i.Istate i.Inaics4 i.Inaics2_time) vce(cluster Inaics4 time)
	...
	twoway (rarea min95 max95 time, fcolor(Blueish-gray8*0.15) lcolor(purple*0.15) lw(none) lpattern(solid)) ///
	       ...
	       ylabel(, format(%9.2f))
		restore
		graph export "$figures/figure6_panelC.eps", replace

*********
**(2.3)**
*********
*Figure 6 panel D: deviation in average monthly earnings for firm with 500+ employees:
qui reghdfe dev_earnbeg5 ib239.t##c.bartik, absorb(i.Istate i.Inaics4 i.Inaics2_time) vce(cluster Inaics4 time)
	...
	twoway (rarea min95 max95 time, fcolor(Blueish-gray8*0.15) lcolor(purple*0.15) lw(none) lpattern(solid)) ///
	       ...
	       ylabel(, format(%9.2f))
		restore
		graph export "$figures/figure6_panelD.eps", replace
